...
if $ONLY_FAILED; then
  echo "Restarting FAILED velop ${SINK_TARGET} tasks against ${CONNECT_URL} (use --all for everything)"
else
  echo "Restarting ALL velop ${SINK_TARGET} connectors + tasks against ${CONNECT_URL}"
fi
