...
This module pulls a handful of tabular / JSON sections out of the dump into
plain ``list[dict]`` records that ``kafka_sink.py`` then produces to per-table
Kafka topics (``velop.device`` / ``wlan_client`` / ``backhaul`` / ``node`` ...),
which the Connect JDBC sinks land in the matching CrateDB tables.
